...
async def _register_org(session, email="owner@example.com", org_name="Inventory Test Org"):
...
def test_update_inventory_config(app_session: AsyncSession):
    async def _run():
        ...
        updated = await inventory_service.update_inventory(
            transaction.inventory_id, InventoryUpdateRequest(minimum_stock=5, maximum_stock=200, reorder_point=8)
        )
        assert updated.minimum_stock == 5
        assert updated.maximum_stock == 200
        assert updated.reorder_point == 8

    asyncio.run(_run())


def test_list_inventory_low_stock_filter(app_session: AsyncSession):
    async def _run():
        user = await _register_org(app_session)
        product, warehouse = await _setup_product_and_warehouse(app_session, user.organization_id, user.id)
        movement_service = StockMovementService(app_session, organization_id=user.organization_id)
        await movement_service.apply_movement(
            product_id=product.id, variant_id=None, warehouse_id=warehouse.id,
            transaction_type=InventoryTransactionType.receive, quantity_delta=5,
        )

        inventory_service = InventoryService(app_session, organization_id=user.organization_id)
        items, _ = await inventory_service.list_inventory(warehouse_id=warehouse.id)
        inventory_id = items[0].id
        await inventory_service.update_inventory(inventory_id, InventoryUpdateRequest(reorder_point=10))

        low_stock_items, low_stock_total = await inventory_service.list_inventory(low_stock_only=True)
        assert low_stock_total == 1
        assert low_stock_items[0].id == inventory_id

    asyncio.run(_run())


def test_list_transactions(app_session: AsyncSession):
    async def _run():
        user = await _register_org(app_session)
        product, warehouse = await _setup_product_and_warehouse(app_session, user.organization_id, user.id)
        movement_service = StockMovementService(app_session, organization_id=user.organization_id)
        ...
        await movement_service.apply_movement(
            product_id=product.id, variant_id=None, warehouse_id=warehouse.id,
            transaction_type=InventoryTransactionType.sale, quantity_delta=-4,
        )

        inventory_service = InventoryService(app_session, organization_id=user.organization_id)
        items, total = await inventory_service.list_transactions(warehouse_id=warehouse.id)
        assert total == 2
        assert {i.type.value for i in items} == {"receive", "sale"}
...
def test_tenant_isolation(app_session: AsyncSession):
    async def _run():
        ...
        await movement_service_a.apply_movement(
        ...
        items_b, total_b = await inventory_service_b.list_inventory()
        assert total_b == 0
        assert items_b == []
