...
import { StockAdjustmentFormModal } from '@/components/inventory/stock-adjustment-form-modal';
import { PermissionGuard } from '@/components/permission-guard';
import { EmptyState } from '@/components/ui/empty-state';
import { FormError } from '@/components/ui/form';
import { SkeletonRows } from '@/components/ui/skeleton';
import { createAdjustment, fetchAdjustments, type StockAdjustmentCreateInput } from '@/lib/inventory/adjustments';
import { fetchWarehouses } from '@/lib/inventory/warehouses';
import type { StockAdjustmentItem, StockAdjustmentReason } from '@/lib/types';

const PAGE_SIZE = 25;

const REASON_STYLES: Record<StockAdjustmentReason, string> = {
  damage: 'text-red-600 dark:text-red-400',
  lost: 'text-red-600 dark:text-red-400',
  found: 'text-emerald-600 dark:text-emerald-400',
  audit: 'text-cyan-600 dark:text-cyan-400',
...
export default function StockAdjustmentsPage() {
  ...
  const load = async () => {
    ...
    try {
      const [data, warehousesData] = await Promise.all([
        fetchAdjustments({ reason: reasonFilter || undefined, limit: PAGE_SIZE, offset }),
      ...
      setError(err instanceof Error ? err.message : 'Unable to load stock adjustments.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (data: StockAdjustmentCreateInput) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold text-slate-900 dark:text-white">Stock Adjustments</h2>
      ...
      {loading ? (
        ...
        <EmptyState icon={SlidersHorizontal} title="No stock adjustments found" description="Manually correct stock levels for damage, loss, found items, or audits." />
      ) : (
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {items.map((item) => (
                <tr key={item.id} className="bg-white dark:bg-slate-900/40">
                  <td className="px-4 py-3 font-medium text-slate-900 dark:text-white">{item.adjustment_number}</td>
                  <td className="px-4 py-3 text-slate-500 dark:text-slate-400">{warehouseNames[item.warehouse_id] ?? item.warehouse_id}</td>
                  <td className={`px-4 py-3 font-medium ${item.quantity_delta < 0 ? 'text-red-600 dark:text-red-400' : 'text-emerald-600 dark:text-emerald-400'}`}>
                    {item.quantity_delta > 0 ? `+${item.quantity_delta}` : item.quantity_delta}
                  </td>
                  <td className="px-4 py-3">
                    <span className={`capitalize ${REASON_STYLES[item.reason]}`}>{item.reason}</span>
                  </td>
      ...
      {showForm ? (
        <StockAdjustmentFormModal onSubmit={handleSave} onClose={() => setShowForm(false)} />
      ) : null}
    </div>
  );
}
