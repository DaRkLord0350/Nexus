...
import { PurchaseOrderFormModal } from '@/components/inventory/purchase-order-form-modal';
import { PermissionGuard } from '@/components/permission-guard';
import { EmptyState } from '@/components/ui/empty-state';
import { FormError, FormInput } from '@/components/ui/form';
import { SkeletonRows } from '@/components/ui/skeleton';
import { createPurchaseOrder, deletePurchaseOrder, fetchPurchaseOrders, type PurchaseOrderCreateInput } from '@/lib/inventory/purchase-orders';
import { fetchWarehouses } from '@/lib/inventory/warehouses';
import type { PurchaseOrderItem, PurchaseOrderStatus, WarehouseItem } from '@/lib/types';

const PAGE_SIZE = 25;

const STATUS_STYLES: Record<PurchaseOrderStatus, string> = {
  draft: 'text-slate-500 dark:text-slate-400',
  sent: 'text-cyan-600 dark:text-cyan-400',
  partially_received: 'text-amber-600 dark:text-amber-400',
...
export default function PurchaseOrdersPage() {
  ...
  const load = async () => {
    ...
    try {
      const [data, warehousesData] = await Promise.all([
        fetchPurchaseOrders({ q: q || undefined, status: statusFilter || undefined, limit: PAGE_SIZE, offset }),
  ...
  const handleSave = async (data: PurchaseOrderCreateInput) => {
    await createPurchaseOrder(data);
    await load();
  };

  const handleDelete = async (po: PurchaseOrderItem) => {
    if (!confirm(`Delete draft purchase order "${po.po_number}"?`)) return;
    try {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold text-slate-900 dark:text-white">Purchase Orders</h2>
        <PermissionGuard permission="inventory.purchase_orders.create">
          <button
            type="button"
      ...
      <div className="grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900/60 sm:grid-cols-3">
        <FormInput placeholder="Search by PO # or supplier…" value={q} onChange={(e) => setQ(e.target.value)} />
        <select
          value={statusFilter}
      ...
      {loading ? (
        ...
        <EmptyState icon={ClipboardList} title="No purchase orders found" description="Create a purchase order to start ordering stock from a supplier." />
      ) : (
              ...
              {items.map((item) => (
                <tr key={item.id} className="bg-white dark:bg-slate-900/40">
                  <td className="px-4 py-3 font-medium text-slate-900 dark:text-white">
                    <Link href={`/dashboard/inventory/purchase-orders/${item.id}`} className="hover:text-cyan-600 dark:hover:text-cyan-300">
                      {item.po_number}
                    </Link>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    {item.status === 'draft' ? (
                      <PermissionGuard permission="inventory.purchase_orders.delete">
      ...
      {showForm ? (
        <PurchaseOrderFormModal warehouses={warehouses} onSubmit={handleSave} onClose={() => setShowForm(false)} />
      ) : null}
    </div>
  );
}
