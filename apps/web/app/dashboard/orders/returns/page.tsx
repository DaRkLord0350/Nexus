...
import { ReturnFormModal } from '@/components/orders/return-form-modal';
import { PermissionGuard } from '@/components/permission-guard';
import { EmptyState } from '@/components/ui/empty-state';
import { FormError, FormInput } from '@/components/ui/form';
import { SkeletonRows } from '@/components/ui/skeleton';
import { createReturn, fetchReturns, type ReturnCreateInput } from '@/lib/orders/returns';
import type { ReturnRequestRead, ReturnStatus } from '@/lib/types';

const PAGE_SIZE = 25;

const STATUS_OPTIONS: ReturnStatus[] = ['requested', 'approved', 'rejected', 'awaiting_pickup', 'in_transit', 'received', 'inspecting', 'completed', 'cancelled'];
...
const STATUS_STYLES: Record<ReturnStatus, string> = {
  ...
  awaiting_pickup: 'text-amber-600 dark:text-amber-400',
  in_transit: 'text-blue-600 dark:text-blue-400',
  received: 'text-blue-600 dark:text-blue-400',
  inspecting: 'text-blue-600 dark:text-blue-400',
...
export default function ReturnsPage() {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const data = await fetchReturns({ q: q || undefined, status: statusFilter || undefined, limit: PAGE_SIZE, offset });
      setItems(data.items);
      setTotal(data.total);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Unable to load return requests.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (data: ReturnCreateInput) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold text-slate-900 dark:text-white">Returns</h2>
      ...
      <div className="grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900/60 sm:grid-cols-3">
        <FormInput placeholder="Search by return #…" value={q} onChange={(e) => setQ(e.target.value)} />
      ...
      {loading ? (
        ...
        <EmptyState icon={RotateCcw} title="No return requests found" description="Return requests from delivered orders will appear here." />
      ) : (
              ...
              {items.map((item) => (
                <tr key={item.id} className="bg-white dark:bg-slate-900/40">
                  <td className="px-4 py-3 font-medium text-slate-900 dark:text-white">
                    <Link href={`/dashboard/orders/returns/${item.id}`} className="hover:text-cyan-600 dark:hover:text-cyan-300">{item.return_number}</Link>
                  </td>
                  <td className="px-4 py-3 capitalize text-slate-600 dark:text-slate-300">{item.reason_code.replace(/_/g, ' ')}</td>
                  <td className="px-4 py-3 capitalize text-slate-500 dark:text-slate-400">{item.resolution?.replace(/_/g, ' ') ?? '—'}</td>
      ...
      {showForm ? <ReturnFormModal onSubmit={handleSave} onClose={() => setShowForm(false)} /> : null}
    </div>
  );
}
